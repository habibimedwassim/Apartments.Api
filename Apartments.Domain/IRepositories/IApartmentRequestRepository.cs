...
public interface IApartmentRequestRepository
{
    ...
    Task UpdateApartmentRequestAsync(ApartmentRequest originalRecord, ApartmentRequest updatedRecord, string userEmail, string[]? additionalPropertiesToExclude = null);
    Task<PagedModel<ApartmentRequest>> GetApartmentRequestsPagedAsync(ApartmentRequestQueryFilter apartmentRequestQueryFilter, RequestType requestType, string id);
    Task<ApartmentRequest?> GetApartmentRequestWithStatusAsync(int apartmentId, string tenantId, string type, string status);
}
